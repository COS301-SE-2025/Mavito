...
from app.db.session import engine  # For DB check and potential table creation
...
async def create_db_and_tables_if_sqlite():
    ...
    if (
        engine
        and settings.SQLALCHEMY_DATABASE_URL
        and "sqlite" in settings.SQLALCHEMY_DATABASE_URL
    ):
        logger.info(
            f"SQLite detected ({settings.SQLALCHEMY_DATABASE_URL.split('@')[-1] if '@' in settings.SQLALCHEMY_DATABASE_URL else settings.SQLALCHEMY_DATABASE_URL}). Attempting to create tables..."
        )
        try:
...
async def on_startup():
    ...
    if (
        db_url_to_log and "@" in db_url_to_log
    ):  # Basic attempt to hide credentials in log
        db_url_to_log = "...@" + db_url_to_log.split("@", 1)[-1]
    logger.info(f"Database URL (potentially masked): {db_url_to_log}")

    if (
        not settings.SECRET_KEY
        or settings.SECRET_KEY == "!!!CONFIG_ERROR_SECRET_KEY_NOT_SET!!!"
        or settings.SECRET_KEY == "!!!SET_SECRET_KEY_IN_ENV_FILE_OR_ENVIRONMENT!!!"
    ):
        logger.critical(
            "SECURITY WARNING: SECRET_KEY is not set or is using a weak default. This is insecure!"
        )
